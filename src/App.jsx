...
function Vitrine({ products, loading, user, onLogout, onShowAuth, config: configProp }) {
  ...
  const filtered = products.filter(p=>{
    ...
    const visible = p.ativo !== false; // show by default unless explicitly hidden
    return m&&c&&visible;
  });
  ...
  return (
    <div style={{fontFamily:"'Inter',sans-serif",background:"#f1f5f9",minHeight:"100vh",maxWidth:600,margin:"0 auto",paddingBottom:90}}>
      <div style={{background:"linear-gradient(135deg,#0f172a 0%,#1e3a5f 100%)",padding:"20px 20px 0",color:"#fff"}}>
        ...
        <div style={{display:"flex",marginTop:16,overflowX:"auto",gap:0,WebkitOverflowScrolling:"touch",scrollbarWidth:"none"}}>
          {[
  ...
  return (
    <div style={{fontFamily:"'Inter',sans-serif",display:"flex",minHeight:"100vh",background:"#f1f5f9"}}>
      ...
      <div style={{marginLeft:240,flex:1,padding:28,minHeight:"100vh"}}>
        ...
        {tab==="vitrine" && (
          <div>
            <div style={{fontSize:22,fontWeight:800,color:"#0f172a",marginBottom:6}}>🏪 Vitrine</div>
            <div style={{color:"#64748b",fontSize:14,marginBottom:20}}>Gerencie quais produtos aparecem para os clientes. Clique em Ocultar/Mostrar para controlar a exibição.</div>
            <AdminVitrineView products={products} onToggle={handleToggleProduct}/>
          </div>
        )}
        {tab==="vendas" && (
          <div>
...
function AdminVitrineView({ products, onToggle }) {
  ...
  const visiveis = products.filter(p=>p.ativo!==false).length;
  const ocultos = products.filter(p=>p.ativo===false).length;

  return (
    <div>
      {/* Stats */}
      <div style={{display:"flex",gap:10,marginBottom:16}}>
        <div style={{background:"#d1fae5",borderRadius:10,padding:"10px 16px",fontSize:13,fontWeight:700,color:"#065f46"}}>
          👁 {visiveis} visível{visiveis!==1?"is":""}
        </div>
        <div style={{background:"#fee2e2",borderRadius:10,padding:"10px 16px",fontSize:13,fontWeight:700,color:"#dc2626"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
        <input placeholder="🔍 Buscar..." value={search} onChange={e=>setSearch(e.target.value)}
          style={{flex:1,minWidth:160,padding:"9px 12px",borderRadius:10,border:"1px solid #e2e8f0",fontSize:14,outline:"none",background:"#fff"}}/>
      </div>
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
      ...
      {/* Grid — same layout as client vitrine */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:12}}>
        {filtered.map(p=>{
          const oculto = p.ativo === false;
          return (
            <div key={p.id} style={{
              background: oculto ? "#f8fafc" : "#fff",
              borderRadius:14,
              boxShadow:"0 1px 4px rgba(0,0,0,0.08)",
              overflow:"hidden",
              opacity: oculto ? 0.6 : 1,
              border: oculto ? "2px dashed #e2e8f0" : "2px solid #e2e8f0",
              display:"flex", flexDirection:"column",
              transition:"all .2s"
            }}>
              {/* Oculto badge */}
              {oculto && (
                <div style={{background:"#fee2e2",padding:"4px 0",textAlign:"center",fontSize:11,fontWeight:700,color:"#dc2626"}}>
                  🚫 Oculto na vitrine
                </div>
              )}
              <div style={{padding:"14px 14px 10px",flex:1}}>
                <div style={{fontSize:13,fontWeight:800,lineHeight:1.3,color:oculto?"#94a3b8":"#0f172a",marginBottom:3,minHeight:34}}>{p.name}</div>
                <div style={{fontSize:12,color:"#6366f1",fontWeight:600}}>{p.flavor}</div>
                <div style={{fontSize:15,fontWeight:800,color:oculto?"#94a3b8":"#0f172a",marginTop:8}}>R$ {Number(p.price).toFixed(2).replace(".",",")}</div>
                <div style={{fontSize:11,color:p.qty===0?"#ef4444":p.qty<=p.min_qty?"#d97706":"#64748b",marginTop:2,fontWeight:600}}>
                  {p.qty===0?"Sem estoque":`${p.qty} em estoque`}
                </div>
              </div>
              <button onClick={()=>onToggle(p)} style={{
                width:"100%", padding:"10px 0", border:"none", fontWeight:700, fontSize:13, cursor:"pointer",
                background: oculto ? "#22c55e" : "#64748b",
                color:"#fff", transition:"background .15s"
              }}>
                {oculto ? "👁 Mostrar na vitrine" : "🚫 Ocultar da vitrine"}
              </button>
            </div>
          );
        })}
